...
Intel-Prozessoren verwenden als kleinste Einheiten einzelne Bytes (8 Bit).
Ein einzelnes Bit muss immer mit dem gesamten Byte verarbeitet werden.
Dargestellt werden die Bytes als 2 hexadezimale Stellen $s_0$ und $s_1$, wobei
die höherwertige Ziffer immer links steht ($s_1 s_0$).
...
#table(
  ...
  bytes-tbl([CA], [FE]), bytes-tbl([CA], [FE], big-endian: false),
  [
...
Der Assembler ist ein Programm, das textuelle Befehle in Maschinencodes übersetzt.
Konventionen hängen massgeblich vom Hersteller des Assemblers ab, selbst für
denselben Prozessor. Wir verwenden in diesem Modul den Netwide Assembler
#link("https://www.nasm.us/xdoc/2.13.01/html/", "NASM").
...
#table(
  ...
  [die ASCII-Codes von `H`, `a`, `l`, `l` und `o`\ $equiv space$```asm db 0x48, 0x61, 0x6c, 0x6c, 0x6f```],
)
Statt einzelnen Bytes können auch Words, DWords und QWords spezifiziert
werden, Endianness muss dann zwingend beachtet werden:
...
`$` bezeichnet den aktuellen Offset, bevor die von der aktuellen Zeile generierten Bytes
in die Ausgabe-Sequenz geschrieben werden.
...
Intern assoziiert der Assembler das Label $w$ mit dem Offset $A(w)$ des nachfolgenden
Befehls in der Ausgabedatei und ersetzt jede Verwendung von $w$ in der Quelle
durch $A(w)$ in der Ausgabe-Sequenz.
...
#bytes-tbl(
  ...
  [00],
  [00],
  extra: (grid.cell(colspan: 8, align(left, ` ^ my_text`)), grid.cell(colspan: 8, align(left, ` ^ current`))),
),
...
/ Importierte Symbole: mit ```asm extern``` deklarierte Label, haben keinen definierten Wert innerhalb der Objekt-Datei
